...
func fetchComments(postID int64, gm *gorm.DB, excludedIDs []string, sort string, uid string, h handler, c *gin.Context, commentSpecificKey string) ([]CommentThreadGroup, error) {
	...
	query := gm.
		...
		WITH top_root_comments AS (
			...
			SELECT c.id, c.post_id, c.vote_score, c.edited, c.trending_score, c.content, c.parent_root, c.created_at, c.updated_at, c.hidden, c.children_count, c.user_id, c.downvote, c.upvote, c.numerical_user, c.numerical_replying_user, c.numerical_replying_user_is_op, c.numerical_user_is_op,
		...
		FROM (
			...
			FROM (
				SELECT id, post_id, vote_score, edited, trending_score, content, parent_root, updated_at, created_at, hidden, user_id, children_count, downvote, upvote, numerical_user, numerical_replying_user, numerical_replying_user_is_op, numerical_user_is_op FROM top_root_comments
				UNION ALL
				SELECT id, post_id, vote_score, edited, trending_score, content, parent_root, created_at, updated_at, hidden, user_id, children_count, downvote, upvote, numerical_user, numerical_replying_user, numerical_replying_user_is_op, numerical_user_is_op
				FROM ranked_replies
				WHERE reply_num <= ?
